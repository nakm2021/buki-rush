...
export function applyGateEffect(stats: PlayerStats, gate: GateOption): PlayerStats {
  ...
  switch (gate.kind) {
    ...
    case 'element':
      ...
      next.power += Math.max(1, gate.value);
      next.tier += 1;
    ...
    case 'module':
      if (gate.module) {
        ...
        next.modules = alreadyOwned ? next.modules : [...next.modules, gate.module];
        next.power += profile.powerBonus + (alreadyOwned ? 1 : 0);
        next.fireRate = Math.max(0.8, next.fireRate + profile.fireBonus);
        next.critRate += gate.module === 'critical' ? 0.08 : 0.015;
        next.pierce += gate.module === 'pierce' ? 1 : 0;
        next.shield += gate.module === 'shield' ? 1 : 0;
        next.synergy += alreadyOwned ? 2 : 1;
      }
      break;
    case 'rarity':
      next.rarity = gate.rarity ?? next.rarity;
      next.power += Math.round(getRarityProfile(next.rarity).multiplier * 2);
      next.tier += 1;
      next.synergy += 2;
      break;
    case 'fusion':
      next.level += 1;
      next.tier += gate.value;
      next.power += gate.value + next.modules.length;
      next.fireRate += 0.18;
      next.critRate += 0.03;
      next.synergy += 3;
      break;
