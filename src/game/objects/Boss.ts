...
import { BOSS_IMAGE_ASSETS, getBossAsset } from '../systems/AssetCatalog';

export class Boss extends Phaser.GameObjects.Container {
  private hp: number;

  constructor(scene: Phaser.Scene, x: number, y: number, hp: number, textureKey = BOSS_IMAGE_ASSETS[0].key) {
    super(scene, x, y);

    this.hp = hp;

    const aura = scene.add.circle(0, 40, 190, 0xff174d, 0.14);
    const imageAsset = getBossAsset(textureKey);
    const image = scene.textures.exists(imageAsset.key) ? scene.add.image(0, 40, imageAsset.key).setDisplaySize(imageAsset.width, imageAsset.height) : undefined;
    ...
    bossBody.setSize(190, 190);
    bossBody.setOffset(-95, -55);
